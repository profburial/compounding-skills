...
description: >
  Audit your generated .claude/ skills and commands — run structured evals to measure whether they actually improve
  Claude's output, then iterate on improvements using the same playbook as Anthropic's skill-creator plugin.
  Use this after running /compounding-skills:setup to verify and improve your generated skills and commands.
disable-model-invocation: true
argument-hint: [skill or command name/path to audit, or leave blank to choose]
---

# compounding-skills:audit — Skill & Command Quality Audit

You are auditing skills and commands generated by `compounding-skills:setup` to verify they actually improve Claude's output quality. This follows the same eval playbook used by Anthropic's official skill-creator plugin.
...
## Phase 1 — Discover Skills & Commands

Find all generated skills and commands in the project's `.claude/` directory:

```bash
# Find all SKILL.md files (skills)
...
If the audit target above specifies a particular skill or command, focus on that one. Otherwise, present everything discovered and ask what to audit:

Use **AskUserQuestion**:
```
question: "What would you like to audit?"
header: "Available skills & commands"
options:
  - (list each discovered skill with its name and description)
  - (list each discovered command with its name and description)
  - label: "All skills"
    description: "Audit every generated skill (takes longer)"
  - label: "All commands"
    description: "Audit every generated command (takes longer)"
  - label: "Everything"
    description: "Audit all skills and commands"
```

For each selected item, read its content fully:
- **Skills**: Read SKILL.md and all reference files to understand what it teaches Claude
- **Commands**: Read the full command file to understand the workflow it defines
...
Test prompts should simulate realistic invocations of the command:
- Match how a real user would invoke the command (with realistic arguments)
- Exercise the command's core phases and decision points
- Cover different scenarios the command should handle (e.g., for a `plan` command: a simple bug fix, a multi-step feature, a refactor)

For a `{prefix}:plan` command, good test prompts might look like:
...
```bash
# The skill-auditor lives alongside this command in the plugin
...
- Also read any skills referenced by the command (e.g., expert-{stack}-developer)
- Follow the command's phases and workflow exactly as written
- Task: {eval prompt}
- Input files: {eval files if any, or "none"}
- Save all outputs to: {workspace}/iteration-{N}/eval-{ID}/with_command/outputs/
- When done, save a transcript of your work as transcript.md in the outputs directory
```
...
- Task: {eval prompt}
- Input files: {eval files if any, or "none"}
- Save all outputs to: {workspace}/iteration-{N}/eval-{ID}/without_command/outputs/
- Do NOT read any command or skill files — complete this task using only your general knowledge
- When done, save a transcript of your work as transcript.md in the outputs directory
```
...
- Which skills and commands were audited
- Pass rate with skill/command vs. without (the delta is its value)
- Key improvements made
- Any skills that didn't outperform baseline (consider simplifying or removing)
- Any commands where the workflow didn't produce better results than ad-hoc execution (consider streamlining phases or removing unnecessary steps)
