...
description: >
  Audit your generated .claude/ skills — run structured evals to measure whether skills actually improve Claude's output,
  then iterate on improvements using the same playbook as Anthropic's skill-creator plugin.
  Use this after running /compounding-skills-setup to verify and improve your generated skills.
disable-model-invocation: true
allowed-tools: Bash, Read, Write, Edit, Glob, Grep, Agent, AskUserQuestion
argument-hint: [skill name or path to audit, or leave blank to choose]
---
...
If the audit target above specifies a particular skill, focus on that one. Otherwise, present the discovered skills and ask which to audit:

Use **AskUserQuestion**:
```
question: "Which skill would you like to audit?"
...
For each selected skill, read its SKILL.md and all reference files to understand what it teaches Claude.

## Phase 2 — Generate Test Cases

For each skill being audited, create 2-3 realistic test prompts that exercise what the skill is supposed to improve. These should be the kind of tasks a real developer would actually ask Claude to do in this project.

The test prompts should:
...
- Pass rate with skill vs. without (the delta is the skill's value)
